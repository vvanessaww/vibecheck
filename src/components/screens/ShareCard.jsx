...
export default function ShareCard({ personaId, onBack }) {
  ...
  return (
    <div className="flex flex-col items-center justify-center w-full h-full p-4" style={{ animation: 'fadeIn 0.5s ease-out forwards', minHeight: '600px' }}>
      <div
        id="share-card"
      ...
      >
        ...
        <div className="z-10 flex flex-col items-center">
          <p className="font-caveat" style={{ color: '#ff5c00', fontSize: '24px', transform: 'rotate(-6deg)', marginBottom: '8px' }}>
            My Festival Persona:
          </p>
          <h2
            className="font-oswald font-black uppercase text-white"
            style={{ fontSize: '42px', letterSpacing: '-0.05em', lineHeight: '0.85' }}
          >
            {persona.title.replace('The ', '')}
          </h2>
          <p className="font-inter font-black uppercase mt-2" style={{ fontSize: '10px', letterSpacing: '0.2em', color: 'rgba(255,255,255,0.6)' }}>
            {persona.subtitle}
          </p>
        </div>
        ...
        <div className="z-10 w-full">
          ...
          <div className="flex flex-wrap justify-center items-center" style={{ gap: '8px' }}>
            {persona.traits.slice(0, 4).map((trait, i) => (
              <span key={trait}>
                <span className="font-inter font-black uppercase text-white" style={{ fontSize: '11px', letterSpacing: '-0.02em' }}>
                  {trait}
                </span>
